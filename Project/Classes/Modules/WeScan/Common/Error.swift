...
extension ImageScannerControllerError: LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .authorization:
            return "Failed to get the user's authorization for camera.".localized
        case .inputDevice:
            return "Could not setup input device.".localized 
        case .capture:
            return "Could not capture pitcure.".localized
        case .ciImageCreation:
            return "Internal Error - Could not create CIImage".localized
        }
    }
