...
final class QuadrilateralView: UIView {
    ...
    var quadLayer: CAShapeLayer = {
        let layer = CAShapeLayer()
        layer.strokeColor = UIColor._green3.cgColor
        layer.lineWidth = 2.0
        layer.opacity = 1.0
        layer.isHidden = true
    ...
    var editable = false {
        didSet {
            ...
            if editable {
                quadLayer.strokeColor = UIColor(red: 1, green: 1, blue: 1, alpha: 0.6).cgColor
                quadLayer.lineWidth = 0
                quadLayer.fillColor =  UIColor(red: 0.093, green: 0.533, blue: 0.202, alpha: 0.7).cgColor

            } else {
                quadLayer.fillColor =  UIColor(red: 0.273, green: 0.762, blue: 0.395, alpha: 0.3).cgColor
            }
    ...
    private var isHighlighted = false {
        didSet (oldValue) {
            ...
            quadLayer.fillColor = isHighlighted ? UIColor.clear.cgColor : UIColor(red: 0.093, green: 0.533, blue: 0.202, alpha: 0.7).cgColor
            isHighlighted ? bringSubviewToFront(quadView) : sendSubviewToBack(quadView) //bug
        }
    }
    ...
    private let highlightedCornerViewSize = CGSize(width: 70, height: 70)
    private let cornerViewSize = CGSize(width: 52, height: 52)
    ...
    private func setupConstraints() {
        ...
        let offset = quadLayer.lineWidth
        quadViewConstraints.forEach { $0.constant = -offset }
    ...
    override public func layoutSubviews() {
        ...
        guard quadLayer.frame != quadView.bounds else {
            return
        }
        
        quadLayer.frame = quadView.bounds
        if let quad = quad {
    ...
    private func drawQuad(_ quad: Quadrilateral, animated: Bool) {
        ...
        if editable {
            path = path.reversing()
            let rectPath = UIBezierPath(rect: quadView.bounds)
