...
public protocol ImageScannerControllerDelegate: NSObjectProtocol {
    ...
    /// Tells the delegate that pick image from Photo albums
    func imageScannerControllerDidPickPhoto(_ scanner: ImageScannerController)
    
    /// Tells the delegate that save all the photos
    func imageScannerControllerDidSave(_ scanner: ImageScannerController)
    
    /// if allowsEditing is false, called when a picture with or without a quadrilateral has been captured
    func imageScannerControllerDidCapture(_ scanner: ImageScannerController, picture: UIImage)
    
    /// Tells the delegate that edit scan has done
...
public final class ImageScannerController: UINavigationController {
    ...
    required init(image: UIImage? = nil, savedQuad: Quadrilateral? = nil, delegate: ImageScannerControllerDelegate? = nil, item: FolderItem? = nil, type: EditPresenterType? = .none) {
        ...
        navigationBar.tintColor = ._green1
        ...
        if let image = image {
            ...
            if #available(iOS 11.0, *) {
                ...
                VisionRectangleDetector.rectangle(forImage: ciImage, orientation: orientation) { (quad) in
                    if let savedQuad = savedQuad {
                        detectedQuad = savedQuad
                    } else {
